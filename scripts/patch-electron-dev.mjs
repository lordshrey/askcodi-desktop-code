// Patches the Electron.app bundle in node_modules to show "AskCodi" name and icon in macOS dock during dev mode.
...
if (existsSync(plistPath)) {
  try {
    execSync(`/usr/libexec/PlistBuddy -c "Set :CFBundleName AskCodi" "${plistPath}"`)
    execSync(`/usr/libexec/PlistBuddy -c "Set :CFBundleDisplayName AskCodi" "${plistPath}"`)
    console.log("[patch-electron-dev] Updated Info.plist: name -> AskCodi")
  } catch (e) {
