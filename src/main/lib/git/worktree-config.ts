...
export type WorktreeConfigSource = "custom" | "cursor" | "askcodi" | null
...
const ONECODE_CONFIG_PATH = ".askcodi/worktree.json"
 ...
 * Priority: custom path > .cursor/worktrees.json > .askcodi/worktree.json
 */
export async function detectWorktreeConfig(
  ...
  if (await fileExists(onecodePath)) {
    ...
    if (config) {
      return { config, path: onecodePath, source: "askcodi" }
    }
  }
